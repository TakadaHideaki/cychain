...
extension UIViewController {
    ...
    private func keyboardWillBeShown(notification: Notification) {
        guard let field = self.view.currentFirstResponder() as? UIView,
        ...
        // textViweの画面上の絶対座標
        let textFieldAbsPoint = field.absPoint
        // 画面サイズ
        let screenSize = UIScreen.main.bounds.size
        // textFieldの底の位置の座標
        let textPosition = textFieldAbsPoint.y + field.frame.height
        // キーボード位置
        let keyboardPosition = screenSize.height - keyboardFrame.size.height
        // 移動判定
        if textPosition >= keyboardPosition {
            // 移動距離
            let offsetY = textPosition - keyboardPosition + 10
            // 移動
