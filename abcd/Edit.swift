...
class Edit: UIViewController, UINavigationControllerDelegate,UITextFieldDelegate, UIImagePickerControllerDelegate, UITextViewDelegate, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        myNameTextField.text = dict["myName"]
        targetNameTextField.text = dict["targetName"]
        messageTextView.text = dict["message"]
        
        messageTextView.tag = 124
        toolBar()
        
        let setImage = image ?? UIImage(named: "user12")
    ...
    @IBAction func senderData(_ sender: Any) {
        ...
        let names_messageArray = [myName, targetName, message]
        var value = ["message": self.messageTextView.text!, "image": ""] as [String : Any]
